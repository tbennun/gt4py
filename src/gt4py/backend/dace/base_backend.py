...
"""
    ...
    def generate_class_members(self):
        ...
        dummy_args = ", ".join("None" for _ in self.sdfg.arglist())
        source = f"""
def __init__(self, *args, **kwargs):
    super().__init__(*args, **kwargs)
    dace_lib["__dace_init_{self.sdfg.name}"]({dummy_args})
    weakref.finalize(self, dace_lib["__dace_exit_{self.sdfg.name}"], {dummy_args})
"""
    ...
    def generate_implementation(self):
        ...
        for arg in self.builder.implementation_ir.api_signature:
            if True or arg.name not in self.builder.implementation_ir.unreferenced:
                args.append(arg.name)
                # if arg.name in self.implementation_ir.fields:
                #     args.append("list(_origin_['{}'])".format(arg.name))
        field_slices = []

        for field_name in self.builder.implementation_ir.arg_fields:
            if field_name not in self.builder.implementation_ir.unreferenced:
                field_slices.append(
...
{name}_ptr = int({name}_interface['data'][0] + sum([(o-e)*st for o, e, st in zip(_origin_['{name}'], {extent}, [_{name}_I_stride*itemsize, _{name}_J_stride*itemsize, _{name}_K_stride*itemsize])]))""".format(
                        ...
                        extent=tuple(
                            -self.builder.implementation_ir.fields_extents[field_name][i][0]
                            for i in range(3)
                        ),
                    )
                )
        ...
        run_args_strs = []
        for name, datadescr in self.sdfg.arglist().items():
            if name in self.builder.implementation_ir.arg_fields:
...
class DaceBackend(gt_backend.BasePyExtBackend):
    name = "dace"
    options = {
    ...
    def generate_dace(self, stencil_id, implementation_ir, options):
        import dace
        ...
        for name, array in sdfg.arrays.items():
            ...
            if isinstance(array, dace.data.Array) and array.transient:
                layout = self.storage_info["layout_map"]([True] * 3)
                stride = 1
                strides = [None] * 3
        ...
        return dace_ext_lib, dace_build_path, sdfg
    ...
    def generate(self):
        ...
        if implementation_ir.has_effect:
            dace_ext_lib, dace_build_path, sdfg = self.generate_dace(
                self.builder.stencil_id, implementation_ir, self.builder.options
            )
        else:
            dace_ext_lib, dace_build_path, sdfg = None, None, None

        return self.make_module(
            dace_ext_lib=dace_ext_lib, dace_build_path=dace_build_path, sdfg=sdfg
        )
