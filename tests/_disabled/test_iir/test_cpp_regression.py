...
def run_vertical_advection_dycore_with_scalar_storage(backend, id_version, domain):
    ...
    masks = {"dtr_stage": (False, False, False)}
    name = "vertical_advection_dycore_with_scalar_storage"

    arg_fields = get_reference(name, backend, domain, origins, shapes, masks)
    validate_fields = {
        name + "_reference": arg_fields.pop(name + "_reference") for name in validate_field_names
    }

    testmodule = generate_test_module(
        "vertical_advection_dycore_with_scalar_storage",
        backend,
